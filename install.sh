...
install() {
  ...
  [[ -d ${SCHEMES_DIR}/We10XOS.colors ]] && rm -r ${SCHEMES_DIR}/We10XOS.colors
  ...
  [[ ${color} == '' ]] && \
  cp -ur ${SRC_DIR}/color-schemes/We10XOS.colors                                    ${SCHEMES_DIR} && \
  cp -ur ${SRC_DIR}/color-schemes/We10XOS.colors                                    ${PLASMA_DIR}/${name}/colors

  [[ ${color} == '-light' ]] && \
  cp -ur ${SRC_DIR}/color-schemes/We10XOS.colors                               ${SCHEMES_DIR}
}

echo "Installing 'We10XOS kde themes'..."
